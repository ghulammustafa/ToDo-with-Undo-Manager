...
- (void) performFetch
{
	...
	NSError *error;
	self.fetchedResultsController = [[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:self.context sectionNameKeyPath:@"sectionName" cacheName:nil];
...
- (void) segmentedControlValueChanged:(id)sender 
{
    ...
    UISegmentedControl *theSegmentedControl = sender;
    NSInteger selectedIndex = theSegmentedControl.selectedSegmentIndex;
    
    if (selectedIndex == 0) {   // Undo
...
- (void) viewDidLoad
{
    ...
    NSArray *anArrayOfItems = [NSArray arrayWithObjects:@"Undo", @"Redo", nil];
    UISegmentedControl *aSegmentedControl = [[UISegmentedControl alloc] initWithItems:anArrayOfItems];
    [aSegmentedControl setSegmentedControlStyle:UISegmentedControlStyleBar];
    [aSegmentedControl setMomentary:YES];
    [aSegmentedControl addTarget:self action:@selector(segmentedControlValueChanged:) forControlEvents:UIControlEventValueChanged];
    self.navigationItem.titleView = aSegmentedControl;
